...
To get started, run
 `yo metaproject` on your project's directory, in this case, `skeleton`
    ...
    [?] Version: 0.0.1
    [?] Comma separated list of authors: Guilherme Barile <gui@bravado.com.br>
...
Then, visit http://server/skeleton to check your initial app.

If you don't have a local webserver to export the application, run

    python -m SimpleHTTPServer 8080

to export the current directory, then visit 
...
This module is loaded in `app/modules/index.js`
...
using a REST Api. In this example
  ...
  * `POST /objectiveweb/product/` Creates a new product, returns its `id`
...
Then, on your terminal run `yo metaproject:crud products Product --menu` to build 
the `products` module using the `Product` model and add it to the sidebar menu
